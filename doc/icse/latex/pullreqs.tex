...
\section{Looking deeper in non merged pull requests}
...
\begin{table}
  ...
  \begin{tabular}{p{6em}p{14em}cc}
    \hline
    \textbf{Reason} & \textbf{Description} & \textbf{Mean \%}
\\
    \hline
    ...
    {\sc pr} or in another branch. &  3.5 \\

    \textsf{superseded} &	A new {\sc pr} solves the problem better. & 9.5\\

    \textsf{process} & The {\sc pr} does not follow the correct project
    conventions for sending and handling pull requests. & 5 \\

    \textsf{duplicate} & The functionality already exists in the project. & 1.5 \\

    \textsf{incorrect implementation} &	The implementation of the feature is
    incorrect, missing or not following project standards. & 12 \\

    \textsf{superfluous} & {\sc pr} doesn't solve an existing problem or add a
    feature needed by the project. & 4 \\

    \textsf{tests} & Tests failed to run. & 2\\
    ...
    \textsf{unknow} & The {\sc pr} could not be classified due to lacking
    information & 23 \\
...
As expected, the results show that there is no clearly outstanding reason for closing pull requests. However, if we group together reasons that have a timing dimension (\textsf{obsolete}, \textsf{conflict}, \textsf{superseded}), we
see that 17.5\% of unmerged pull requests are closed due to concurrent
modifications of the code in project branches. Moreover, for 23\% of the pull requests the human examiners could not identify the cause of not merging them. 
This usually means that there was no discussion prior to closing the pull
request or the pull request were automatically initiated and managed by 
external tools; incidentally all projects that had such pull requests in our
random sample did not use Github's issue tracking facilities. Finally,
22.5\% of the pull requests were actually merged; this means that the proportion
of merged pull requests reported in this study (84\%) in slightly underrated, due to insufficient heuristics. By extrapolation, the total number of merged pull
requests could be as high as 90\%.
...
location in the project's Github page. Moreover, a project owner can invest in a
