...
\textbf{2. Using Git merge.} When a pull request cannot be applied cleanly or
    ...
    squashing}: if the full history is not of interest to the project,
...
(versus, e.g., clones), and what is the relation to Github's issue tracking
facilities. The outcomes are presented in Section~\ref{sec:ghtorrent}.
...
qualitatively analyze in-depth a set of randomly chosen non-merged pull
requests. We used open coding (a grounded theory tool) to come up with an
inclusive set of tags of why pull requests are not merged as follows: the first
author read the pull request discussion on Github for randomly selected pull
requests and summarized the reasons for closing them into one sentence per
sample; during a second pass, the descriptions were aggregated and codes were
extracted. To validate the identified codes, all three authors applied them on a
different set of pull requests, compared results, identified inconsistencies and
retrofitting the initial selection of codes. The final set of codes was then
applied on a third sample and we used that sample to draw results from. The
sampling process is described in Section~\ref{sec:qualdata}.
...
The post processing phase left us with 14 features, which can be seen in
...
\emph{RQ1: 14\% of repositories are using pull requests on Github. Pull request usage is increasing in absolute numbers, even though the proportion of repositories using them is relatively stable.}
\end{minipage}}
%
...
developer has submitted to the same project, the lowest the time to process each
one of them.
...
\section{Factors influencing merging and merging time}
\label{sec:accrej}
...
\texttt{num\_comments} and \texttt{num\_commits} features as they could not be
measured reliably at the pull request arrival time. Based on the results
presented in Table~\ref{tab:classif-perf}, we selected the \texttt{randomforest}
...
and \textsf{more than a day} classes respectively. For the
\textsf{mergedecision} experiment, the prior probability for the dominant class
was 84\% which allowed the algorithm to achieve near perfect scores. In both
cases, the stability of the {\sc auc} metric across folds was good
(\textsf{mergetime}: $\sigma_{auc} = 0.019$, \textsf{mergedecision}:
$\sigma_{auc} = 0.008$).
...
that 37\% of unmerged pull requests are closed due to concurrent modifications
of the code in project branches.  Another 18\% (\textsf{superfluous},
...
submitting uninteresting changes. 24\% of the contributions are rejected with
reasons that have to do with project process and quality requirements
(\textsf{process}, \textsf{incorrect implementation}, \textsf{tests}); this may
...
\fbox{
\begin{minipage}{0.46\textwidth}
  \emph{RQ4: 37\% of pull requests are discarded due to concurrent
  modifications of code in project branches and forks while 24\% are rejected
  due to project process and quality requirements not being met.}
\end{minipage}}
