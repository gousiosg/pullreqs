...
classes (\textsf{hour}, \textsf{day}, \textsf{rest}) according to the time
...
\begin{figure}[t]
  \centering
\includegraphics[scale=0.45]{pullreqs-per-month}
...
is highly skewed.  Projects such as Ruby on Rails, the Homebrew package manager
have more that 5,000 pull requests.  From the pull requests that have been
opened in 2013, 73,07\% have been merged using Github facilities, thereby
indicating that pull requests in principle can work as a means for obtaining
external contributions. Moreover, even though one might expect that it is the
well known projects that receive most pull requests, this is only moderately
supported by our data: the Spearman rank correlation between the number of stars
of a project and the number of pull requests it has received is $\rho = 0.36$
($p < 0.001, n = 239,131$).
...
pull requests are not merged. For that reason, we manually looked into 250 pull
