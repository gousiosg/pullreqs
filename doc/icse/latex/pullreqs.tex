...
\begin{table}
  \begin{small}
  \centering
  \begin{tabular}{p{6em}p{18em}c}
    \hline
    \textbf{Reason} & \textbf{Description} & \textbf{Cases} \\
    \hline
    \textsf{obsolete} &	The {\sc pr} is no longer relevant, as the project
    has progressed. \\

    \textsf{process} & The {\sc pr} does not follow the correct project
    conventions for sending and handling pull requests. \\

    \textsf{conflict} &	There feature is currently being implemented by other
    {\sc pr} or in another branch. \\

    \textsf{superseded} &	A newer {\sc pr} solves the problem better.\\

    \textsf{duplicate} & The functionality already exists in the project. \\

    \textsf{incorrect implementation} &	The implementation of the feature is
    incorrect, missing or not following project standards. \\

    \textsf{superfluous} & {\sc pr} doesn't solve an existing problem or add a
    feature needed by the project. \\

    \textsf{tests} & Tests failed to run. \\
