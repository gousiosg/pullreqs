...
distributed software development is carried out. Driven by pragmatic needs,
most {\sc dvcs}s were designed from scratch to work as advanced patch management
...
application of which leads to the current state. The change sets can
...
\textbf{Pull requests} 
    ...
    specifies a local branch to be merged with a branch in the main repository.
    A member of the project's core team is then responsible to inspect the
    changes and pull them to the project's master branch. If changes are
    considered unsatisfactory, more changes may be requested; in that case,
...
through automatic comparison of project branches. Similarly to Git-based pull
requests, a Github pull request contains a branch from which the main repository
owner should pull commits from. An example pull request on Github can be seen 
in Figure~\ref{fig:pullreq-scr}.

\begin{figure}[t]
  \centering
\subfigure[The pull request process.]{
\includegraphics[scale=0.5]{pullreq}
\label{fig:pullreq-process}
}
%\subfigure[The different states of a pull request.]{
%    \includegraphics[scale=0.5]{pr-state-machine.pdf}  
%  \label{fig:state}
%}
\subfigure[An example Github cross-branch pull request (edited for space). The
participants first interact in a code review, the result of which is the update
of the pull request through a commit. A second reviewer then approves the pull
request, which is merged by the first author.]{
   \includegraphics[scale=0.23]{pullreq-scr.png}
   \label{fig:pullreq-scr}
}
\caption{Processing pull requests on Github}
\end{figure}

By default, pull requests are submitted to the base repository for inspection.
When 

The list of commits is automatically updated when newer
commits have been added to the forked repository after the pull request has been
created.
...
be part of the main project team. The versatility of  Git suite enables
...
\textbf{Committing the patch.} 
  ...
  request branch, which then applies to the upstream branch. Both history and
...
\begin{figure}
  \begin{center}
    \includegraphics[scale=0.5]{cross-cor.pdf}
  \end{center}
