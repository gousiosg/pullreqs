...
(Section~\ref{sec:featureselection}).
...
offered through the Github {\sc api}. The data come in two forms; a streaming
...
the {\sc api}. The {\sc ght}orrent dataset covers a broad range of development
...
\begin{description}
  ...
  \item [Committing the patch] The merger creates a
...
%\begin{description}
  ...
  \textbf{Project characteristics} features quantify how receptive to pull
  ...
  ``yesterday's weather'' change pattern~\cite{Girba04}), so it natural to
...
directly correlated to the project's size ($\rho = -0.23$) nor to the project's
test coverage ($\rho = 0.34$) Moreover, projects are not getting faster at pull
request processing by processing more; the correlation between the average time
to merge and the number of pull requests is weak ($\rho = -0.33, n = 88, p <
0.01$).
...
\fbox{
\begin{minipage}{0.46\textwidth}
\emph{RQ2: The factors that primarily affect the decision to merge a pull request are: i)
...
\fbox{
\begin{minipage}{0.46\textwidth}
\emph{RQ3: The factors that primarily affect the time to merge a pull request
