...
data offered by the Github {\sc api}, the \pullreqs dataset includes many features extracted by combining \ghtorrent and the project's repository and is offered in a format ready to be processed by statistical software.
...
The selected features are split into three categories:
  ...
  commits. \todo{Do you have a reference?}
  ...
  participants \todo{the number of reviewers?} has been shown to influence the time to process \todo{process the change suggestions?} of code
  reviewing~\cite{Rigby13}. Therefore, the number of comments in pull requests
  is split between code review and discussion. \todo{I don't see the consequence that you are implying with ``therefore''... am I missing something?}
  ...
  Bird et al. \todo{reference?} presented evidence that social reputation has an
  impact on whether a patch will be merged; in our dataset, the number of
  followers on Github can be seen as a proxy for reputation \todo{Why can this be seen as a proxy?}.
...
the following heuristics \todo{How frequent is this merging outside Github?}, listed here in order of application:
...
both in source code and also in API examples embedded in documentation comments. \todo{Is this a side-effect? Is this a feature or a bug? :-)}
...
\begin{itemize}
  ...
  \item Projects should have at least one commit coming from a developer outside
    ...
    the project. No projects were excluded due to this criterion. \todo{just 
		curious, how can you see if someone is from outside the main team?}
