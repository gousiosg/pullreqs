...
```{r plot5, message=FALSE, fig.align='center', echo=FALSE, fig.width=9}
  ...
  res <- dbSendQuery(db, sprintf(unwrap(q), pid))
  df <- fetch(res, n = -1)
  df$opened <- as.POSIXct(df$opened)
  df$closed <- as.POSIXct(df$closed)

  perc.09 <- as.numeric(quantile(df$closed - df$opened, 0.9))
  df <- subset(df, closed - opened > perc.09)
...
For this project, the cutoff is `r perc.09 / 3600 /24 ` days.
The line represents the time between opening and closing the pull request.
