...
rfmodel <- randomForest(merged_fast~. - mergetime_minutes - num_comments - requester, data=a, importance = T)
print(rfmodel)
#summary(rfmodel)
...
svmmodel <- svm(merged_fast~. - mergetime_minutes - requester, data=a, gamma=bestGamma, cost = bestCost, cross=10)
summary(svmmodel)
...
lmmodel <- lm(log_mergetime ~ watchers + sloc + test_lines_per_1000_lines + num_comments + perc_external_contribs + requester_succ_rate + team_size + prev_pullreqs, data=a)
step <- stepAIC(lmmodel, direction="both")
step$anova # display results 
summary(step$model)

# model of stepwise regression
lmmmodel <- lm(log_mergetime ~ num_comments + perc_external_contribs + requester_succ_rate + team_size + prev_pullreqs, data = a)
summary(lmmodel)
